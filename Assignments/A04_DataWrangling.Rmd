...
#8
EPA.AirPollutants <- EPA.AirPollutants.wrangled %>%
  intersect(EPA.AirPollutants$Site.Name) %>%
  group_by(Date, Site.Name, AQS_PARAMETER_DESC, COUNTY) %>%
  mean(DAILY_AQI_VALUE, SITE_LATITUDE, SITE_LONGITUDE) %>%
    mutate(Month = month(Date)) %>%
  mutate(Year = year(Date))
# Go over with Njeri how to write this pipe in office hours 

#9
EPA.AirPollutants.spread <- spread(EPA.AirPollutants.wrangled, DAILY_AQI_VALUE)
