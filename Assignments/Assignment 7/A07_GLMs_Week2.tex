...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Chemistry.raw <-}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"./Data/Raw/NTL-LTER_Lake_ChemistryPhysics_Raw.csv"}\NormalTok{)}
...
\begin{Highlighting}[]
\CommentTok{#3}
\NormalTok{Chemistry.wrangled <-}\StringTok{ }
\StringTok{  }\NormalTok{Chemistry.raw }\OperatorTok{%>%}
...
\CommentTok{#4}
\NormalTok{Chemistry.AIC <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Chemistry.wrangled, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{year4 }\OperatorTok{+}\StringTok{ }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }\NormalTok{depth)}
\KeywordTok{step}\NormalTok{(Chemistry.AIC)}
\end{Highlighting}
\end{Shaded}
...
## 
## Call:
## lm(formula = temperature_C ~ year4 + daynum + depth, data = Chemistry.wrangled)
## 
## Coefficients:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Chemistry.model <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Chemistry.wrangled, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{year4 }\OperatorTok{+}\StringTok{ }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }\NormalTok{depth)}
\KeywordTok{summary}\NormalTok{(Chemistry.model)}
\end{Highlighting}
\end{Shaded}
...
are year, day number, and depth. Year, day number, and depth all
significantly predict temperature (multiple linear regression: F=9303,
df=3 and 9718, p\textless{}0.0001). This model explains 74.2\% of the
observed variance in temperature (R2=0.7417).
\end{quote}
...
\begin{Highlighting}[]
\CommentTok{#6}
\NormalTok{Chemistry.ancova.interaction <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Chemistry.wrangled, temperature_C }\OperatorTok{~}\StringTok{ }\NormalTok{lakename }\OperatorTok{*}\StringTok{ }\NormalTok{depth)}
\KeywordTok{summary}\NormalTok{(Chemistry.ancova.interaction)}
\end{Highlighting}
\end{Shaded}
...
## 
## Call:
## lm(formula = temperature_C ~ lakename * depth, data = Chemistry.wrangled)
## 
## Residuals:
##     Min      1Q  Median      3Q     Max 
...
\begin{quote}
Answer: Yes, there is a significant interaction between depth and lake
name (ANCOVA: F=2097, df=17 and 9704, p\textless{}0.0001). This model
explains 78.6\% of observed variation in temperature (R2=0.7857).
\end{quote}
...
\NormalTok{Tempplot <-}\StringTok{ }\KeywordTok{ggplot}\NormalTok{(Chemistry.wrangled, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ depth, }\DataTypeTok{y =}\NormalTok{ temperature_C, }\DataTypeTok{color =}\NormalTok{ lakename)) }\OperatorTok{+}
...
\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \StringTok{"Depth (m)"}\NormalTok{, }\DataTypeTok{y =} \StringTok{"Temperature (C)"}\NormalTok{, }\DataTypeTok{color =} \StringTok{"Lake name"}\NormalTok{) }\OperatorTok{+}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Litter.mixed <-}\StringTok{ }\KeywordTok{lme}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Litter.processed,}
...
\begin{quote}
Answer: 26.8\% of variance in dry mass is explained by the model when
the random effect is added (as compared to 24.7\% without the random
effect). As such, about 2.13\% more variance is explained by adding the
random effect to the model.
\end{quote}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{Litter.fixed <-}\StringTok{ }\KeywordTok{gls}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ Litter.processed,}
\NormalTok{                      dryMass }\OperatorTok{~}\StringTok{ }\NormalTok{nlcdClass }\OperatorTok{*}\StringTok{ }\NormalTok{functionalGroup)}
...
\begin{quote}
Answer: The mixed effects model is the better model because it has a
lower AIC value (p\textless{}0.0001). This means that the mixed effects
model is more parsimonious while also accounting for a good amount of
