...
NTL.nutrient.data.PeterPaul <- 
  NTL.nutrient.data.PeterPaul %>% #
  mutate(month = month(sampledate)) %>% # Add a month column
  select(lakeid:daynum, month, sampledate:comments) %>% # 
  drop_na(depth)
dim(NTL.nutrient.data.PeterPaul) # Dropped NAs so dimensions are smaller
...
# Writing above code as more succinct code
NTL.nutrient.data.PeterPaul.subset <-select(NTL.nutrient.data.PeterPaul, 
...
NTL.nutrient.data.PeterPaul.dates <- separate(NTL.nutrient.data.PeterPaul.subset, 
                                              sampledate, c("Y", "m", "d"))
View(NTL.nutrient.data.PeterPaul.dates)
