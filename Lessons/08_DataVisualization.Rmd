...
ggplot(EPAair, aes(x = Date, y = Ozone)) + 
  geom_point()
# In ggplot, use a + instead of a , to say you want to move to the next line
...
# Tell R to make Year a factor rather than an integer because it needs to have a categorical variable to change color based on year
PMplot <- 
...
# Here can also use %in% c(list of things you want to include), subset works similiarly to filter function, so set up the same argument
PMplot.faceted2 <- 
...
# Can use facet_grid in place of facet_wrap, different arguments though
Temp.ByDay <-
...
# Adjust axes
# Supplying axis limit values gets rid of outliers 
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) +
...
# Set method as linear model (method =lm), smooth just gives a best fit line, shows confidence interval of the best fit line but can put se = FALSE if you want to get rid of it - one line here is a best fit line of the data and the other is an expected line of what it the data should look like based on research
NvsP <-
...
# 1. 
AirQual <-
  ggplot(EPAair, aes(x = Ozone, y = PM2.5, color = SITE_LATITUDE, apha = 0.5)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE) 
print(AirQual)
