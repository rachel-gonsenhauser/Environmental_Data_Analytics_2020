...
# This is setting mytheme as your default plot and will apply it to all plots so you don't need to keep calling up the theme throughout the session
#Helpful to change text size in the custom theme you make
theme_set(mytheme)
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
# ggplot adds things in layers so whatever you want to show up on top, add it last
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) + # different lty values change line type
  geom_hline(yintercept = 100, lty = 2) +
  geom_point(alpha = 0.5, size = 1.5) + # alpha = 0.5 means the points are 50% transparent
  ...
  geom_text(x = as.Date("2020-01-01"), y = 120, label = "unhealthy (sensitive groups)", hjust = 1, fontface = "bold") + # hjust justifies on either the right of left
...
# Redo this later, this is all wrong
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
...
Nutrientplot <-
  ...
  geom_boxplot() +
    labs(x = expression(paste("")),
         y = expression(paste("TN (", mu, "g/L)")),
                color = "Nutrients", shape = "Nutrients") +
