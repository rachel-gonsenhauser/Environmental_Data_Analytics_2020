...
NCAir <- read.csv("./Data/Processed/EPAair_O3_PM25_NC1819_Processed_KS.csv")
...
# drop_na only drops rows where NAs are present in a specified column of data whereas na.exclude and na.omit take away all rows in whole dataset where there is an NA

EnoDischarge.early <- EnoDischarge.complete %>%
  filter(datetime < as.Date("1985-01-01"))
# When you filter you still need to tell R to format as a date
...
# For ts function, double square brackets allows you to take just data from one column (8tth column of dataset)
```
...
# Need to input a ts object, can't just provide the dataframe
...
> Mean daily discharge in the Eno River from 1927-1971 was significantly higher than mean daily discharge from 1985-2019 (Wilcoxon test, W = 1.17*10^17, p<0.0001). You could use a violin plot or a boxplot to graph the difference between the mean discharge of early and later years.
...
> aggregation: taking multiple data points and making one data point from them (e.g. taking mean of monthly data to get annual average), would use if you have a lot of data and want to aggregate into a longer time span

> subsampling: taking a subset of the existing dataset (e.g. if you have daily data, you could take the first day of each month to subsample to get one point for each month OR if you're missing data for a bunch of years, only using years with complete cases) , this method is probably used the least in time series analysis

> interpolation: filling missing cases/data gaps in the dataset (examples below)
...
# Spline interpolation can also be specified as an alternative, defaults to linear interpolation
...
>There is not a significant monotonic/seasonal trend in discharge data at the Eno River over the period of 1985-2019 (seasonal Mann-Kendall: z=-0.13967, p=0.8889). We did not detect any trends across individual months in the dataset.
