...
# Test for normality (of raw data, not model fit residuals)
...
# Plot ID is a significant predictor of dry mass, means are different across plots

# Format ANOVA as lm
Litter.Totals.anova2 <- lm(data = Litter.Totals, dryMass ~ plotID)
summary(Litter.Totals.anova2)
# Output is noting whether that specific plot ID is significantly different from intercept

# Checking model fit and assumptions
# ANOVA is robust against departures from normality.
plot(Litter.Totals.anova2)

# Post-hoc test, only works if you run it on your aov object, not lm, gives all pairwise relationships between plots
TukeyHSD(Litter.Totals.anova)

# Extract groupings for pairwise relationships, tell it which variable is our predictor variable (plotID) and tell it to give you groupinngs (grouup = TRUE), if letters match between groups, they are not statistically significant (any two with an a are not significantly different)
...
# For stat summary, tell it which type of geometry to plot, where to place the text (max y value here), vertically adjust by -1 so it doesn't plot on top of value, adjust size of letters, and assign labels and put in exact order they appear in dataset, will be different order that it appears in the HSD test)
...
# May need to reset y limits of graph when you use stat summary
  ylim(0, 85)
...
> Total litter biomass differe significantly among sites at the Niwot Ridge LTER (Figure 1; ANOVA,F11,198= 4.813, p<0.0001).
...
Litter.anova.plot <- ggplot(Litter, aes(y = dryMass, x = nlcdClass, color = functionalGroup)) +
  ...
  labs(x = "NLCD Class", y = "Dry Mass (g)") +
