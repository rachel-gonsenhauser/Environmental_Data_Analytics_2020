...
my_proj4 <- "+proj=aea +lat_1=29.5 +lat_2=45.5 +lat_0=23 +lon_0=-96 +x_0=0 +y_0=0 +datum=NAD83 +units=m +no_defs"
my_epsg <- 5070
...
# st_as_sf, go into help file to look up
...
ggplot() +
  geom_sf(data = southeast_state_bounds) + # you can add color = "blue" to change outline color to blue or lwd = 2 to change the width of the lines, set the boundary as the first layer of data, then the points can be the second layer on top of the base
...
pdf(here("outputs", "nc_basins.pdf"), width = 11, height = 8.5)
ggplot() +
...
# get basemap
nc_basemap <- get_map(nc_bbox_fix, maptype = 'terrain-background', source = 'stamen', zoom = 8) # a bit more zoomed out than city scale (10)
class(nc_basemap)
st_crs(nc_basemap)
...
# need to mutate tabular so they can join, look later
