...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# If trying to load tidyverse make sure message = FALSE}
...
\CommentTok{# Can't find because we all have different machines, so do not use absolute file path}

\CommentTok{# Relative file path (friendly for users regardless of machine)}
\CommentTok{# . means go in one folder, .. means go out one folder}
\CommentTok{# Want to do this every time rather than just uploading the dataset from folders because this is reproducible}
...
\CommentTok{# What happens if we don't assign a name to our imported dataset? Just comes up in the console}
\CommentTok{#read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv")}

\CommentTok{# Another option is to choose with your browser, not reproducible though so not recommended}
...
file was found in the Metadata folder. The information in the README
file relates to best practices for reproducible data analysis because it
steps through exactly how to download the data from the USGS site. Also,
naming conventions are given to provide a systematic way to collaborate
on files.
...
\KeywordTok{class}\NormalTok{(USGS.flow.data) }\CommentTok{#dataframe because it has different types of data and is a two dimensional dataset}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Rename columns to make more straightforward, need to list all column names}
\CommentTok{# If you only want to rename certain columns, need to subset e.g. colnames(USGS.flow.data)[3] <-c("datetime") if you want to just change column 3}
\CommentTok{#Try not to go over 80 characters per line, put returns between col names}
...
\KeywordTok{str}\NormalTok{(USGS.flow.data) }\CommentTok{# Asking for the structure of the data frame}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{dim}\NormalTok{(USGS.flow.data) }\CommentTok{# Dimensions of data frame (# rows and columns)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{length}\NormalTok{(USGS.flow.data) }\CommentTok{# Length gives you the number of columns}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{nrow}\NormalTok{(USGS.flow.data) }\CommentTok{# Number of rows in data frame}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{summary}\NormalTok{(USGS.flow.data) }\CommentTok{# Summary stats for dataset}
\end{Highlighting}
\end{Shaded}
...
into R? \textgreater{} Answer: Come up as NA
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{USGS.flow.data.complete <-}\StringTok{ }\KeywordTok{na.omit}\NormalTok{(USGS.flow.data) }\CommentTok{# If you remove an NA, it omits the entire row from the dataset, will only include the complete cases within the data frame}
...
day-month-year is used in an ambiguously formatted spreadsheet. If R
prompts you to set to origin, you should set this as January 1, 1970.
...
\CommentTok{# Need to tell R the format the date exists in right now in the dataset, not the format we want it in}
\CommentTok{# Column datetime coded as $datetime}
\end{Highlighting}
\end{Shaded}
...
\CommentTok{# Building a function to fix dates from 2000s to 1999s}
...
\CommentTok{# Created processed data file once you make a folder for it to live in}
\end{Highlighting}
\end{Shaded}
