...
# Shapiro test can only take up to 5000 values, which is why we're subsetting, this tells R that it's taking a random assortment of 5000 observatioons

# Evaluate assumption of normal distribution
# Null hypothesis of shaprio-wilk test is that the data are normally distributed
shapiro.test((EPAair.subsample$Ozone))
ggplot(EPAair, aes(x = Ozone)) +
  geom_histogram() 
# Data are right skewed, long right tail
qqnorm(EPAair$Ozone); qqline(EPAair$Ozone)
# Tells R to run both lines of code at the same time, adds a qqline to show what the data should look like if the data are normally distributed (line represents normal distribution)

# Run a t-test (mu = mean), alternative = "less" means your alternative hypothesis is that the mean is less than 50, if you don't specify an alternative R will assume you want to run a two-tailed test
...
Ozone.plot <- ggplot(EPAair, aes(x = Ozone)) +
  ...
  # Density plot only needs an x variable
  geom_density(fill = "gray") +
  geom_vline(xintercept = 50, color = "#238b45", lty = 2, size = 0.9) +
  scale_x_continuous(expand = c(0, 0)) + scale_y_continuous(expand = c(0, 0))
# This final line of code specifies that the axes should start right at 0,0 or else R starts them a bit up from the origin
print(Ozone.plot)
```
...
> At an alpha level of 0.05, we can reject the null hypothesis that the mean AQI index is equal to 50 (t=-57.98, df=6829, p<0.0001). As such, we have sufficient evidence that the mean AQI value is significantly lower than 50 and that ozone levels are "good", as they fall below this threshold.  

> Kateri's wording: EPA ozone measurements for 2018-2019 were significantly lower than 50, the AQI threshold for "good" air quality (one sample t-test; t=-57.98, p<0.0001). 
...
# Variance test (~ means "by")

ggplot(EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_freqpoly()
# List year as a factor because otherwise it would be treated as a continuous variable, want to just have color in two disrete groups (2018 and 2019)

# Format as a t-test
O3.twosample <- t.test(EPAair$Ozone ~ EPAair$Year)
# Did not specify an alternative hypotehsis, so R will just see if value is different (not necessarily greater or smaller)
O3.twosample
...
# For a linear model, want to call up a summary of the object
...
O3.onesample.wilcox
# This gives us less statistical robustness since we don't get df
...
```{r}
#1
DensityPlot <- ggplot(EPAair) +
   geom_density(aes(x = Ozone, fill = as.factor(Year)), alpha = 0.5)
print(DensityPlot)

#2
Boxplot <- ggplot(EPAair) +
  geom_boxplot(aes(x = as.factor(Year), y = Ozone)) 
print(Boxplot) 

#3
ViolinPlot <- ggplot(EPAair) +
  geom_violin(aes(x = as.factor(Year), y = Ozone, draw.quantiles = 0.5))
print(ViolinPlot)
...
> EPA ozone measurements for 2019 are significantly higher than measurements in 2018 (two-sample t-test: t=-2.266, df=6467, p=0.007736). The DensityPlot exhibits this increase in ozone concentrations from 2018 to 2019.    
