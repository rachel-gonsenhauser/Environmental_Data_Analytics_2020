...
> Add notes here: May want to see how temperature, dissolved oxygen, and light availability are changing over time, as these have ramifications for species living in the lake, may want a better understanding of how climate change is impacting temperature over time, this information/better understanding of these mechanisms can inform management techniques to mitigate these changes, gives an understanding of species' tolerance to changes in these variables
...
head(NTL.phys.data) # First six rows of each column in data frame
summary(NTL.phys.data) 

str(NTL.phys.data) # Structure will define which type of data is in each column, but otherwise pretty similar to summary function
dim(NTL.phys.data)

class(NTL.phys.data$sampledate)
# Format sampledate as date
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, format = "%m/%d/%y") # Provide current format of data, should usually check that dates are formatted correctly because R often doesn't recognize them as dates
```
...
```{r, results = "hide"} 
vignette("dplyr") # Many of the new packages have vignettes, documents in the Help section that have best practices/advice for how to use, code chunk starts with results = hide so that the vignette doesn't come up when knitting file
```
...
`==` equals (need two equals signs in the filter function in dplyr)
`!=` not equal to 
`<` less than 
...
# matrix filtering 
NTL.phys.data.surface1 <- NTL.phys.data[NTL.phys.data$depth == 0,] # need to specify rows and columns, so need a comma if in matrix notation
...
# %n% says I want to include anything that contains Paul Lake or Peter Lake (concatonating a list)

# Choose a range of conditions of a numeric or integer variable
summary(NTL.phys.data$daynum) # Sampling from day 55 to 307
NTL.phys.data.JunethruOctober1 <- filter(NTL.phys.data, daynum > 151 & daynum < 305) # Needs to fulfill both assumptions
NTL.phys.data.JunethruOctober2 <- filter(NTL.phys.data, daynum > 151, daynum < 305) # Using a comma does the same as above, but more straighforward to use &
NTL.phys.data.JunethruOctober3 <- filter(NTL.phys.data, daynum >= 152 & daynum <= 304)
NTL.phys.data.JunethruOctober4 <- filter(NTL.phys.data, daynum %in% c(152:304)) # Does same thing but just with the includes function
...
view(NTL.phys.data.1999)
summary(NTL.phys.data.1999$year4)

# Exercise: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
NTL.phys.data.90s <- filter(NTL.phys.data, 
                            year4 %in% c(1990:1999)
                            & lakename == "Tuesday Lake")
view(NTL.phys.data.90s)
summary(NTL.phys.data.90s$lakename)
summary(NTL.phys.data.90s$year4)
```
Question: Why don't we filter using row numbers?

> Answer: Want it to filter based on attributes rather than row numbers, way more time consuming and more room for error to filter using row numbers, if the dataset changes then incorrect
...
```{r}
NTL.phys.data.depth.ascending <- arrange(NTL.phys.data, depth) # Arrange dataset by depth values, automatically arranges by ascending value if not otherwise specified
...
# High temperature is associated with low /shallow depth, summer months, at East Long Lake, Hummingbird Lake, and Central Long Lake
...
# Names the column we want to keep or take out of the dataset (in this case, want to include lakename, and columns sampledate through temperature)
# Different from filter function because just put commas between columns rather than logical operators in the filter function

# If you don't want to include lake name
NTL.phys.data.temps <- select(NTL.phys.data, -lakename)
# Cannot mix inclusive and exclusive statements
```
### Mutate
...
# These do the same thing but the latter is more reproducible
```
...
# Need to have dplyr loaded to work with pipes
# When using pipe, can just put your dataset name first followed by %>% (then), and then don't need to lead every function with the dataset name  
# Pipe operator just saves time, faster than making separate functions, very universally used
```
